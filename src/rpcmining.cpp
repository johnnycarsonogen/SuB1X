...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or sub1x.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SUB1X is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SUB1X is downloading blocks...");
...
// Release the wallet and main lock while waiting
        ...
        // TODO: Maybe recheck connections/IBD and (if something wrong) sub1x an expires-immediately template to stop miners?
    }
