...
bool fSub1xFreeTransactions = false;
...
bool CWallet::IsChange(const CTxOut& txout) const
{
    ...
    // a better way of identifying which outputs are 'the sub1x' and which are
...
void CWallet::Resub1xWalletTransactions()
{
    // Do this infrequently and randomly to avoid giving away
    // that these are our transactions.
    if (GetTime() < nNextResub1x)
        return;
    bool fFirst = (nNextResub1x == 0);
    nNextResub1x = GetTime() + GetRand(30 * 60);
    if (fFirst)
        return;

    // Only do it if there's been a new block since last time
    if (nTimeBestReceived < nLastResub1x)
        return;
    nLastResub1x = GetTime();

    // Rebroadcast any of our txes that aren't in a block yet
    LogPrintf("Resub1xWalletTransactions()\n");
    {
        LOCK(cs_wallet);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 SUB1X
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (int64_t v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 SUB1X over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 SUB1X+1 ( bit on if present )
            // bit 1 - 1000 SUB1X+1
            // bit 2 - 100 SUB1X+1
            // bit 3 - 10 SUB1X+1
            // bit 4 - 1 SUB1X+1
...
bool CWallet::SelectCoinsCollateral(std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        // collateral inputs will always be a multiple of DARSUB1X_COLLATERAL, up to five
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    int64_t nFeeRet = 0;
    std::string strFail = "";
    vector<pair<CScript, int64_t> > vecSub1x;
    vecSub1x.push_back(make_pair(scriptChange, BUDGET_FEE_TX));

    CCoinControl* coinControl = NULL;
    bool success = CreateTransaction(vecSub1x, tx, reservekey, nFeeRet, strFail, coinControl, ALL_COINS, useIX, (CAmount)0);
    if (!success) {
...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecSub1x,
    CWalletTx& wtxNew,
    ...
    CAmount nFeePay)
{
    ...
    BOOST_FOREACH (const PAIRTYPE(CScript, CAmount) & s, vecSub1x) {
        if (nValue < 0) {
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 SUB1X.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 SUB1X.");
                    } else {
                        strFailReason = _("Unable to locate enough Obfuscation denominated funds for this transaction.");
                        strFailReason += " " + _("Obfuscation uses exact denominated amounts to sub1x funds, you might simply need to anonymize some more coins.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-sub1x-address
                    CScript scriptChange;

                    // coin control: sub1x change to custom address
                    if (coinControl && !boost::get<CNoDestination>(&coinControl->destChange))
                        scriptChange = GetScriptForDestination(coinControl->destChange);

                    // no coin control: sub1x change to newly generated address
                    else {
                ...
                *static_cast<CTransaction*>(&wtxNew) = CTransaction(txNew);
                ...
                if (fSub1xFreeTransactions && nBytes <= MAX_FREE_TRANSACTION_CREATE_SIZE) {
                    ...
                    // Small enough, and priority high enough, to sub1x for free
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, CAmount nFeePay)
{
    ...
    vecSub1x.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSub1x, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, coin_type, useIX, nFeePay);
}
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins) {
        ...
        if (CheckStakeKernelHash(nBits, block, *pcoin.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, true)) {
            ...
            //presstab HyperStake - if MultiSub1x is set to sub1x in coinstake we will add our outputs here (values asigned further down)
...
string CWallet::PrepareObfuscationDenominate(int minRounds, int maxRounds)
{
    ...
    */
    ...
    obfuScationPool.Sub1xObfuscationDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
...
void CWallet::AutoCombineDust()
{
    ...
    for (map<CBitcoinAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
        ...
        vecSub1x.push_back(make_pair(scriptPubKey, nTotalRewardsValue));
        ...
        //get the fee amount
        CWalletTx wtxdummy;
        CreateTransaction(vecSub1x, wtxdummy, keyChange, nFeeRet, strErr, coinControl, ALL_COINS, false, CAmount(0));
        vecSub1x[0].second = nTotalRewardsValue - nFeeRet - 500;

        if (!CreateTransaction(vecSub1x, wtx, keyChange, nFeeRet, strErr, coinControl, ALL_COINS, false, CAmount(0))) {
...
bool CWallet::MultiSub1x()
{
    if (IsInitialBlockDownload() || IsLocked()) {
        return false;
    }

    if (chainActive.Tip()->nHeight <= nLastMultiSub1xHeight) {
        LogPrintf("Multisub1x: lastmultisub1xheight is higher than current best height\n");
        return false;
    }
    ...
    int mnSent = 0;
    BOOST_FOREACH (const COutput& out, vCoins) {
        //need output with precise confirm count - this is how we identify which is the output to sub1x
        if (out.tx->GetDepthInMainChain() != COINBASE_MATURITY + 1)
            continue;

        COutPoint outpoint(out.tx->GetHash(), out.i);
        bool sub1xMSonMNReward = fMultiSub1xMasternodeReward && outpoint.IsMasternodeReward(out.tx);
        bool sub1xMSOnStake = fMultiSub1xStake && out.tx->IsCoinStake() && !sub1xMSonMNReward; //output is either mnreward or stake reward, not both

        if (!(sub1xMSOnStake || sub1xMSonMNReward))
            continue;

        CTxDestination destMyAddress;
        if (!ExtractDestination(out.tx->vout[out.i].scriptPubKey, destMyAddress)) {
            LogPrintf("Multisub1x: failed to extract destination\n");
            continue;
        }

        //Disabled Addresses won't sub1x MultiSub1x transactions
        if (vDisabledAddresses.size() > 0) {
            for (unsigned int i = 0; i < vDisabledAddresses.size(); i++) {
                if (vDisabledAddresses[i] == CBitcoinAddress(destMyAddress).ToString()) {
                    LogPrintf("Multisub1x: disabled address preventing multisub1x\n");
                    return false;
                }
            }
        }

        // create new coin control, populate it with the selected utxo, create sub1xing vector
        ...
        // loop through multisub1x vector and add amounts and addresses to the sub1xing vector
        const isminefilter filter = ISMINE_SPENDABLE;
        int64_t nAmount = 0;
        for (unsigned int i = 0; i < vMultiSub1x.size(); i++) {
            // MultiSub1x vector is a pair of 1)Address as a std::string 2) Percent of stake to sub1x as an int
            nAmount = ((out.tx->GetCredit(filter) - out.tx->GetDebit(filter)) * vMultiSub1x[i].second) / 100;
            CBitcoinAddress strAddSub1x(vMultiSub1x[i].first);
            CScript scriptPubKey;
            scriptPubKey = GetScriptForDestination(strAddSub1x.Get());
        ...
        CWalletTx wtxdummy;
        string strErr;
        CreateTransaction(vecSub1x, wtxdummy, keyChange, nFeeRet, strErr, cControl, ALL_COINS, false, CAmount(0));
        CAmount nLastSub1xAmount = vecSub1x[vecSub1x.size() - 1].second;
        if (nLastSub1xAmount < nFeeRet + 500) {
            LogPrintf("%s: fee of %s is too large to insert into last output\n");
            return false;
        }
        vecSub1x[vecSub1x.size() - 1].second = nLastSub1xAmount - nFeeRet - 500;

        // Create the transaction and commit it to the network
        if (!CreateTransaction(vecSub1x, wtx, keyChange, nFeeRet, strErr, cControl, ALL_COINS, false, CAmount(0))) {
            LogPrintf("MultiSub1x createtransaction failed\n");
            return false;
        }

        if (!CommitTransaction(wtx, keyChange)) {
            LogPrintf("MultiSub1x transaction commit failed\n");
            return false;
        } else
            fMultiSub1xNotify = true;

        delete cControl;

        //write nLastMultiSub1xHeight to DB
        CWalletDB walletdb(strWalletFile);
        nLastMultiSub1xHeight = chainActive.Tip()->nHeight;
        if (!walletdb.WriteMSettings(fMultiSub1xStake, fMultiSub1xMasternodeReward, nLastMultiSub1xHeight))
            LogPrintf("Failed to write MultiSub1x setting to DB\n");

        LogPrintf("MultiSub1x successfully sent\n");
        if (sub1xMSOnStake)
            stakeSent++;
        else
            mnSent++;
        ...
        if (mnSent > 0 && stakeSent > 0)
            return true;
        if (stakeSent > 0 && !fMultiSub1xMasternodeReward)
            return true;
        if (mnSent > 0 && !fMultiSub1xStake)
            return true;
    }
