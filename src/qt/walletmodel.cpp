...
WalletModel::Sub1xCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    CAmount total = 0;
    QList<Sub1xCoinsRecipient> recipients = transaction.getRecipients();
    std::vector<std::pair<CScript, CAmount> > vecSub1x;
    ...
    foreach (const Sub1xCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            for (int i = 0; i < details.outputs_size(); i++) {
                ...
                vecSub1x.push_back(std::pair<CScript, CAmount>(scriptPubKey, out.amount()));
            }
            if (subtotal <= 0) {
                return InvalidAmount;
            }
            total += subtotal;
        } else { // User-entered sub1x address / amount:
            ...
            vecSub1x.push_back(std::pair<CScript, CAmount>(scriptPubKey, rcp.amount));
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Sub1x Coins"), tr("SwiftTX doesn't support sub1xing values that high yet. Transactions are currently limited to %1 SUB1X.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSub1x, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useSwiftTX);
        ...
        if (!fCreated) {
            if ((total + nFeeRequired) > nBalance) {
                return Sub1xCoinsReturn(AmountWithFeeExceedsBalance);
            }
            emit message(tr("Sub1x Coins"), QString::fromStdString(strFailReason),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
    ...
    return Sub1xCoinsReturn(OK);
}

WalletModel::Sub1xCoinsReturn WalletModel::sub1xCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const Sub1xCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal sub1x:URI (sub1x:XyZ...?message=example)
            {
    ...
    foreach (const Sub1xCoinsRecipient& rcp, transaction.getRecipients()) {
        ...
        if (!rcp.paymentRequest.IsInitialized()) {
            ...
            std::string strLabel = rcp.label.toStdString();
            {
                ...
                if (mi == wallet->mapAddressBook.end()) {
                    wallet->SetAddressBook(dest, strLabel, "sub1x");
