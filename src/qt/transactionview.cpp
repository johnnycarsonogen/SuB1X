                                                    ...
                                                    transactionView(0)
{
    ...
    typeWidget->addItem(tr("Sent to"), TransactionFilterProxy::TYPE(TransactionRecord::Sub1xToAddress) | TransactionFilterProxy::TYPE(TransactionRecord::Sub1xToOther));
    ...
    typeWidget->addItem(tr("To yourself"), TransactionFilterProxy::TYPE(TransactionRecord::Sub1xToSelf));
...
void TransactionView::editLabel()
{
    ...
    if (!selection.isEmpty()) {
        ...
        if (idx != -1) {
            // Edit sub1xing / receiving address
            QModelIndex modelIdx = addressBook->index(idx, 0, QModelIndex());
            // Determine type of address, launch appropriate editor dialog type
            QString type = modelIdx.data(AddressTableModel::TypeRole).toString();

            EditAddressDialog dlg(
                type == AddressTableModel::Receive ? EditAddressDialog::EditReceivingAddress : EditAddressDialog::EditSub1xingAddress, this);
            dlg.setModel(addressBook);
            dlg.loadRow(idx);
            dlg.exec();
        } else {
            // Add sub1xing address
            EditAddressDialog dlg(EditAddressDialog::NewSub1xingAddress,
                this);
            dlg.setModel(addressBook);
