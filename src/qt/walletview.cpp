                                          ...
                                          walletModel(0)
{
    ...
    sub1xCoinsPage = new Sub1xCoinsDialog();

    addWidget(overviewPage);
    addWidget(transactionsPage);
    addWidget(receiveCoinsPage);
    addWidget(sub1xCoinsPage);
    ...
    // Pass through messages from sub1xCoinsPage
    connect(sub1xCoinsPage, SIGNAL(message(QString, QString, unsigned int)), this, SIGNAL(message(QString, QString, unsigned int)));
...
{
    if (gui) {
        // Clicking on a transaction on the overview page simply sub1xs you to transaction history page
...
void WalletView::setClientModel(ClientModel* clientModel)
{
    ...
    sub1xCoinsPage->setClientModel(clientModel);
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void WalletView::setWalletModel(WalletModel* walletModel)
{
    ...
    sub1xCoinsPage->setModel(walletModel);
...
void WalletView::gotoSub1xCoinsPage(QString addr)
{
    setCurrentWidget(sub1xCoinsPage);

    if (!addr.isEmpty())
        sub1xCoinsPage->setAddress(addr);
}
...
void WalletView::gotoMultiSub1xDialog()
{
    MultiSub1xDialog* multiSub1xDialog = new MultiSub1xDialog(this);
    multiSub1xDialog->setModel(walletModel);
    multiSub1xDialog->show();
}

bool WalletView::handlePaymentRequest(const Sub1xCoinsRecipient& recipient)
{
    return sub1xCoinsPage->handlePaymentRequest(recipient);
}
...
void WalletView::usedSub1xingAddresses()
{
    if (!walletModel)
        return;
    AddressBookPage* dlg = new AddressBookPage(AddressBookPage::ForEditing, AddressBookPage::Sub1xingTab, this);
