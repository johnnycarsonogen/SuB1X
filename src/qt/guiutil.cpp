...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SUB1X address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
bool parseBitcoinURI(const QUrl& uri, Sub1xCoinsRecipient* out)
{
    // return if URI is not valid or is no SUB1X: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::SUB1X, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinURI(QString uri, Sub1xCoinsRecipient* out)
{
    // Convert sub1x:// to sub1x:
    //
    //    Cannot handle this later, because sub1x:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const Sub1xCoinsRecipient& info)
{
    QString ret = QString(URI_SCHEME ":%1").arg(info.address);
    int paramCount = 0;

    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SUB1X, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open sub1x.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SUB1X.lnk";
}
...
{
    return GetAutostartDir() / "sub1x.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a sub1x.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SUB1X\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the sub1x app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add sub1x app to startup item list
