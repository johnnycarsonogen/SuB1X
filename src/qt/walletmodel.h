...
{
public:
    explicit Sub1xCoinsRecipient() : amount(0), nVersion(Sub1xCoinsRecipient::CURRENT_VERSION) {}
    explicit Sub1xCoinsRecipient(const QString& addr, const QString& label, const CAmount& amount, const QString& message) : address(addr), label(label), amount(amount), message(message), nVersion(Sub1xCoinsRecipient::CURRENT_VERSION) {}
...
public:
    ...
    enum StatusCode // Returned by sub1xCoins
    {
        OK,
        InvalidAmount,
        InvalidAddress,
    ...
    // Return status record for Sub1xCoins, contains error id + information
    struct Sub1xCoinsReturn {
        Sub1xCoinsReturn(StatusCode status = OK) : status(status) {}
        StatusCode status;
    };

    // prepare transaction for getting txfee before sub1xing coins
    Sub1xCoinsReturn prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl = NULL);

    // Sub1x coins to a list of recipients
    Sub1xCoinsReturn sub1xCoins(WalletModelTransaction& transaction);
...
signals:
    ...
    void coinsSent(CWallet* wallet, Sub1xCoinsRecipient recipient, QByteArray transaction);
