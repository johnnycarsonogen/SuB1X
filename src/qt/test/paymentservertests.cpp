...
static Sub1xCoinsRecipient handleRequest(PaymentServer* server, std::vector<unsigned char>& data)
{
    RecipientCatcher sigCatcher;
    QObject::connect(server, SIGNAL(receivedPaymentRequest(Sub1xCoinsRecipient)),
        &sigCatcher, SLOT(getRecipient(Sub1xCoinsRecipient)));
    ...
    // and sub1x a file open event to the object
    QObject object;
    object.installEventFilter(server);
    QFileOpenEvent event(f.fileName());
    // If sub1xing the event fails, this will cause sigCatcher to be empty,
    // which will lead to a test failure anyway.
    QCoreApplication::sub1xEvent(&object, &event);

    QObject::disconnect(server, SIGNAL(receivedPaymentRequest(Sub1xCoinsRecipient)),
        &sigCatcher, SLOT(getRecipient(Sub1xCoinsRecipient)));
...
void PaymentServerTests::paymentServerTests()
{
    ...
    Sub1xCoinsRecipient r = handleRequest(server, data);
    QString merchant;
...
void RecipientCatcher::getRecipient(Sub1xCoinsRecipient r)
{
    recipient = r;
}
