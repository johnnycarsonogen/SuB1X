...
{
    // This should only be used for sub1xing addresses, never for receiving addresses,
...
//presstab HyperStake
bool CWalletDB::WriteMultiSub1x(std::vector<std::pair<std::string, int> > vMultiSub1x)
{
    nWalletDBUpdated++;
    bool ret = true;
    for (unsigned int i = 0; i < vMultiSub1x.size(); i++) {
        std::pair<std::string, int> pMultiSub1x;
        pMultiSub1x = vMultiSub1x[i];
        if (!Write(std::make_pair(std::string("multisub1x"), i), pMultiSub1x, true))
            ret = false;
    }
    return ret;
}
//presstab HyperStake
bool CWalletDB::EraseMultiSub1x(std::vector<std::pair<std::string, int> > vMultiSub1x)
{
    nWalletDBUpdated++;
    bool ret = true;
    for (unsigned int i = 0; i < vMultiSub1x.size(); i++) {
        std::pair<std::string, int> pMultiSub1x;
        pMultiSub1x = vMultiSub1x[i];
        if (!Erase(std::make_pair(std::string("multisub1x"), i)))
            ret = false;
    }
    return ret;
}
//presstab HyperStake
bool CWalletDB::WriteMSettings(bool fMultiSub1xStake, bool fMultiSub1xMasternode, int nLastMultiSub1xHeight)
{
    nWalletDBUpdated++;
    std::pair<bool, bool> enabledMS(fMultiSub1xStake, fMultiSub1xMasternode);
    std::pair<std::pair<bool, bool>, int> pSettings(enabledMS, nLastMultiSub1xHeight);
...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, string& strType, string& strErr)
{
    try {
        ...
        } else if (strType == "multisub1x") //presstab HyperStake
        {
            unsigned int i;
            ssKey >> i;
            std::pair<std::string, int> pMultiSub1x;
            ssValue >> pMultiSub1x;
            if (CBitcoinAddress(pMultiSub1x.first).IsValid()) {
                pwallet->vMultiSub1x.push_back(pMultiSub1x);
            }
        } else if (strType == "msettingsv2") //presstab HyperStake
        {
            std::pair<std::pair<bool, bool>, int> pSettings;
            ssValue >> pSettings;
            pwallet->fMultiSub1xStake = pSettings.first.first;
            pwallet->fMultiSub1xMasternodeReward = pSettings.first.second;
            pwallet->nLastMultiSub1xHeight = pSettings.second;
