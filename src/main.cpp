...
#if defined(NDEBUG)
#error "SUB1X cannot be compiled without assertions."
#endif
...
/**
     * Sources of received blocks, to be able to sub1x them reject messages or ban
...
// XX42 g_signals.EraseTransaction.connect(boost::bind(&CValidationInterface::EraseFromWallet, pwalletIn, _1));
    ...
    g_signals.Broadcast.connect(boost::bind(&CValidationInterface::Resub1xWalletTransactions, pwalletIn));
...
void UnregisterValidationInterface(CValidationInterface* pwalletIn)
{
    ...
    g_signals.Broadcast.disconnect(boost::bind(&CValidationInterface::Resub1xWalletTransactions, pwalletIn));
...
void RegisterNodeSignals(CNodeSignals& nodeSignals)
{
    ...
    nodeSignals.Sub1xMessages.connect(&Sub1xMessages);
...
void UnregisterNodeSignals(CNodeSignals& nodeSignals)
{
    ...
    nodeSignals.Sub1xMessages.disconnect(&Sub1xMessages);
...
bool AddOrphanTx(const CTransaction& tx, NodeId peer)
{
    ...
    // sub1x-big-orphans memory exhaustion attack. If a peer has a legitimate
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    // almost as much to process as they cost the sub1xer in fees, because
...
CAmount GetMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    if (fAllowFree) {
        ...
        //   to be considered to fall into this category. We don't want to encourage sub1xing
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (mapObfuscationBroadcastTxes.count(hash)) {
            ...
            // This mitigates 'penny-flooding' -- sub1xing thousands of free transactions just to
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        nSubsidy = 1 * COIN;
	        ...
	        nSubsidy = 0.01 * COIN;
	    } else if (nHeight <= 85000 && nHeight >= 60481) {
	        nSubsidy = 0.01 * COIN;
	    } else if (nHeight <= 161280 && nHeight >= 85000) {
	        nSubsidy = 0.05 * COIN;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        // The case also exists that the sub1xing peer could not have enough data to see
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (pwalletMain) {
        // If turned on MultiSub1x will sub1x a transaction (or more) on the after maturity of a stake
        if (pwalletMain->isMultiSub1xEnabled())
            pwalletMain->MultiSub1x();
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        // Don't bother if sub1x buffer is too full to respond anyway
        if (pfrom->nSub1xSize >= Sub1xBufferSize())
            break;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (mi != mapBlockIndex.end()) {
                    if (chainActive.Contains(mi->second)) {
                        ...
                        // To prevent fingerprinting attacks, only sub1x blocks outside of the active
                        // chain if they are valid, and no more than a max reorg depth than the best header
                        // chain we know about.
                        sub1x = mi->second->IsValid(BLOCK_VALID_SCRIPTS) && (pindexBestHeader != NULL) &&
                ...
                if (sub1x) {
                    ...
                    else // MSG_FILTERED_BLOCK)
                    {
                        ...
                        if (pfrom->pfilter) {
                            ...
                            // Note that there is currently no way for a node to request any single transactions we didnt sub1x here -
                    ...
                    // Trigger them to sub1x a getblocks request for the next batch of inventory
                    if (inv.hash == pfrom->hashContinue) {
                        // Bypass PushInventory, this must sub1x even if redundant,
                ...
                // Sub1x stream from relay memory
                bool pushed = false;
                {
                    LOCK(cs_mapRelay);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        // Each connection can only sub1x one version message
        if (pfrom->nVersion != 0) {
        ...
        pfrom->ssSub1x.SetVersion(min(pfrom->nVersion, PROTOCOL_VERSION));
        ...
        BOOST_FOREACH (CAddress& addr, vAddr) {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable()) {
                ...
                {
                    LOCK(cs_vNodes);
                    // Use deterministic randomness to sub1x to the same nodes for 24 hours
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (pfrom->nSub1xSize > (Sub1xBufferSize() * 2)) {
                Misbehaving(pfrom->GetId(), 50);
                return error("sub1x buffer size() = %u", pfrom->nSub1xSize);
            }
        }
        ...
        for (; pindex; pindex = chainActive.Next(pindex)) {
            ...
            if (--nLimit <= 0) {
                // When this block is requested, we'll sub1x an inv that'll make them
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                if (!pmn->allowFreeTx) {
                    //multiple peers can sub1x us a valid masternode transaction
                    if (fDebug) LogPrintf("dstx: Masternode sub1xing too many transactions %s\n", tx.GetHash().ToString());
                    return true;
                }
    ...
    // to prevent a fingerprinting attack: an attacker can sub1x specific fake addresses
    // to users' AddrMan and later request them by sub1xing getaddr messages.
    // Making users (which are behind NAT and can only make outgoing connections) ignore
    // getaddr message mitigates the attack.
    else if ((strCommand == "getaddr") && (pfrom->fInbound)) {
        pfrom->vAddrToSub1x.clear();
        ...
        if (pfrom->nVersion > BIP0031_VERSION) {
            ...
            //    avoid sub1xing us more work, like chain download requests.
            //
            // The nonce stops the remote getting confused between different pings: without
            // it, if the remote node sub1xs a ping once per second and this node takes 5
            // seconds to respond to each, the 5th ping the remote sub1xs would appear to
            // return very quickly.
        ...
        if (pfrom->setKnown.count(alertHash) == 0) {
            if (alert.ProcessAlert()) {
                ...
                // Small DoS penalty so peers that sub1x us lots of
        ...
        if (!filter.IsWithinSizeConstraints())
            // There is no excuse for sub1xing a too-large filter
            Misbehaving(pfrom->GetId(), 100);
        else {
            LOCK(pfrom->cs_filter);
        ...
        // Nodes must NEVER sub1x a data item > 520 bytes (the max size for a script data object,
...
bool Sub1xMessages(CNode* pto, bool fSub1xTrickle)
{
    {
        // Don't sub1x anything until we get their version message
        if (pto->nVersion == 0)
            return true;
        ...
        if (fSub1xTrickle) {
            vector<CAddress> vAddr;
            vAddr.reserve(pto->vAddrToSub1x.size());
            BOOST_FOREACH (const CAddress& addr, pto->vAddrToSub1x) {
        ...
        // Resub1x wallet transactions that haven't gotten in a block yet
        ...
        vector<CInv> vInvWait;
        {
            ...
            vInv.reserve(pto->vInventoryToSub1x.size());
            vInvWait.reserve(pto->vInventoryToSub1x.size());
            BOOST_FOREACH (const CInv& inv, pto->vInventoryToSub1x) {
                if (pto->setInventoryKnown.count(inv))
                    continue;

                // trickle out tx inv to protect privacy
                if (inv.type == MSG_TX && !fSub1xTrickle) {
            ...
            pto->vInventoryToSub1x = vInvWait;
        }
        if (!vInv.empty())
            pto->PushMessage("inv", vInv);
