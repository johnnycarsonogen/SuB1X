...
{
public:
    /// Is the inputs associated with this public key? (and there is 10000 SUB1X - checking if valid masternode)
...
public:
    ...
    /// As a client, sub1x a transaction to a Masternode to start the denomination process
    void Sub1xObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    ...
    /// Get the denominations for a specific amount of sub1x.
