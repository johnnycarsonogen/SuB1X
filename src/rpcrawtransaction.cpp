...
Value getrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "           \"sub1xaddress\"        (string) sub1x address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
Value listunspent(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of sub1x addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) sub1x address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"address\" : \"address\",  (string) the sub1x address\n"
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH (Value& input, inputs) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid SUB1X address: ") + input.get_str());
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "\nCreate a transaction spending the given inputs and sub1xing to the given addresses.\n"
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the sub1x address, the value is the btc amount\n"
            "      ,...\n"
            "    }\n"
...
Value decoderawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) sub1x address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
Value sub1xrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "sub1xrawtransaction \"hexstring\" ( allowhighfees )\n"
            ...
            "\nSub1x the transaction (signed hex)\n" + HelpExampleCli("sub1xrawtransaction", "\"signedhex\"") +
            "\nAs a json rpc call\n" + HelpExampleRpc("sub1xrawtransaction", "\"signedhex\""));
