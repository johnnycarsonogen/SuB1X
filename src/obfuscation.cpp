...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        if (dsq.ready) {
            ...
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("obfuscation", "dsq -- Masternode sub1xing too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
...
//
// Check the Obfuscation progress and sub1x client updates if a Masternode
//
...
void CObfuscationPool::ChargeFees()
{
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't sub1x us the promised transaction
            if (!found) {
                LogPrintf("CObfuscationPool::ChargeFees -- found uncooperative node (didn't sub1x transaction). Found offence.\n");
                offences++;
            }
        }
    }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't sub1x us the promised transaction
            if (!found && r > target) {
                LogPrintf("CObfuscationPool::ChargeFees -- found uncooperative node (didn't sub1x transaction). charging fees.\n");
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat SUB1X and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 SUB1X per transaction on average.
            */
            if (r <= 10) {
...
//
void CObfuscationPool::Sub1xObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount)
{
    if (fMasterNode) {
        LogPrintf("CObfuscationPool::Sub1xObfuscationDenominate() - Obfuscation from a Masternode is not supported currently.\n");
        return;
    }

    if (txCollateral == CMutableTransaction()) {
        LogPrintf("CObfuscationPool:Sub1xObfuscationDenominate() - Obfuscation collateral not set");
        return;
    }
    ...
    if (!sessionFoundMasternode) {
        LogPrintf("CObfuscationPool::Sub1xObfuscationDenominate() - No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
    ...
    if (!CheckDiskSpace()) {
        ...
        LogPrintf("CObfuscationPool::Sub1xObfuscationDenominate() - Not enough disk space, disabling Obfuscation.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CObfuscationPool::Sub1xObfuscationDenominate() - Added transaction to pool.\n");
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeSub1xAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                if (pnode != NULL) {
                    ...
                    LogPrintf("DoAutomaticDenominating --- connected (from queue), sub1xing dsa for %d - %s\n", sessionDenom, pnode->addr.ToString());
                    strAutoDenomResult = _("Mixing in progress...");
        ...
        while (i < 10) {
            ...
            if (pnode != NULL) {
                ...
                pnode->PushMessage("dsa", sessionDenom, txCollateral);
                LogPrintf("DoAutomaticDenominating --- connected, sub1xing dsa for %d\n", sessionDenom);
                strAutoDenomResult = _("Mixing in progress...");
                return true;
            } else {
...
bool CObfuscationPool::Sub1xRandomPaymentToSelf()
{
    ...
    // ****** Add fees ************ /
    vecSub1x.push_back(make_pair(scriptChange, nPayment));

    CCoinControl* coinControl = NULL;
    bool success = pwalletMain->CreateTransaction(vecSub1x, wtx, reservekey, nFeeRet, strFail, coinControl, ONLY_DENOMINATED);
    if (!success) {
        LogPrintf("Sub1xRandomPaymentToSelf: Error - %s\n", strFail);
        return false;
    }

    pwalletMain->CommitTransaction(wtx, reservekey);

    LogPrintf("Sub1xRandomPaymentToSelf Success: tx %s\n", wtx.GetHash().GetHex());
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    vecSub1x.push_back(make_pair(scriptCollateral, OBFUSCATION_COLLATERAL * 4));

    // try to use non-denominated and not mn-like funds
    bool success = pwalletMain->CreateTransaction(vecSub1x, wtx, reservekeyChange,
    ...
    if (!success) {
        ...
        success = pwalletMain->CreateTransaction(vecSub1x, wtx, reservekeyChange,
...
bool CObfuscationPool::CreateDenominated(int64_t nTotalValue)
{
    ...
    BOOST_REVERSE_FOREACH (int64_t v, obfuScationDenominations) {
        ...
        while (nValueLeft - v >= OBFUSCATION_COLLATERAL && nOutputs <= 10) {
            ...
            vecSub1x.push_back(make_pair(scriptDenom, v));
    ...
    // if we have anything left over, it will be automatically sub1x back as change - there is no need to sub1x it manually
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100SUB1X+1 ( bit on if present )
    // bit 1 - 10SUB1X+1
    // bit 2 - 1SUB1X+1
    // bit 3 - .1SUB1X+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("sub1x-obfuscation");
