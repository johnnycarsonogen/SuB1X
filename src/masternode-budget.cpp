...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    if (!mapCollateralTxids.count(tempBudget.GetHash())) {
        ...
        // Sub1x the tx to the network. Do NOT use SwiftTx, locking might need too much time to propagate, especially for testnet
...
void CBudgetManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    /*
        ...
        budget object to see if they're OK. If all checks pass, we'll sub1x it to the peer.
