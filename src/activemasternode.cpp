...
//
// Bootup the Masternode, look for a 10000 SUB1X input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    //need correct blocks to sub1x ping
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (service.GetPort() != 43172) {
                notCapableReason = strprintf("Invalid port: %u - only 43172 is supported on mainnet.", service.GetPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if (service.GetPort() == 43172) {
            notCapableReason = strprintf("Invalid port: %u - 43172 is only supported on mainnet.", service.GetPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
    ...
    //sub1x to all peers
    if (!Sub1xMasternodePing(errorMessage)) {
...
bool CActiveMasternode::Sub1xMasternodePing(std::string& errorMessage)
{
    if (status != ACTIVE_MASTERNODE_STARTED) {
    ...
    LogPrintf("CActiveMasternode::Sub1xMasternodePing() - Relay Masternode Ping vin = %s\n", vin.ToString());
    ...
    if (pmn != NULL) {
        if (pmn->IsPingedWithin(MASTERNODE_PING_SECONDS, mnp.sigTime)) {
            errorMessage = "Too early to sub1x Masternode Ping";
            return false;
        }
        ...
        // Seems like we are trying to sub1x a ping while the Masternode is not registered in the network
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (service.GetPort() != 43172) {
            errorMessage = strprintf("Invalid port %u for masternode %s - only 43172 is supported on mainnet.", service.GetPort(), strService);
            LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
            return false;
        }
    } else if (service.GetPort() == 43172) {
        errorMessage = strprintf("Invalid port %u for masternode %s - 43172 is only supported on mainnet.", service.GetPort(), strService);
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }
