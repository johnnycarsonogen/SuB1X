SUB1X Core version 2.3.0 is now available from:
...
[
  {
    ...
    winner: {
        address: addr,    (string) SUB1X MN Address,
...
The third parameter indicates if the wallet should be unlocked for staking and anonymization only (true), or to allow sub1x operations (false, full unlock).

ZeroMQ (ZMQ) Notifications
--------------

sub1xd can now (optionally) asynchronously notify clients through a ZMQ-based PUB socket of the arrival of new transactions and blocks. This feature requires installation of the ZMQ C API library 4.x and configuring its use through the command line or configuration file. Please see [docs/zmq.md](/doc/zmq.md) for details of operation.
...
SUB1X Core now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
...
- #195 `aee05fe` [ZMQ] ZMQ integration for SUB1X (Mrs-X)
...
### Tests and QA
- #191 `3a778c3` [Tests] Fix the unit test suite for use with SUB1X (Fuzzbawls)
