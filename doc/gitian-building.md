...
*Setup instructions for a gitian build of SUB1X using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SUB1X
...
Clone the git repositories for sub1x and gitian and then checkout the sub1x version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/sub1x-crypto/sub1x.git
cd sub1x
...
Follow the instructions in [doc/release-process.md](release-process.md) in the sub1x repository
...
To build SUB1X (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the sub1x repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sub1x/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/sub1x-crypto/sub1x
...
For example:
```bash
URL=https://github.com/crowning-/sub1x.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit sub1x=${COMMIT} --url sub1x=${URL} ../sub1x/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sub1x=${COMMIT} --url sub1x=${URL} ../sub1x/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sub1x=${COMMIT} --url sub1x=${URL} ../sub1x/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sub1x-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sub1x-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/sub1x-build.assert
```
...
[sub1x/gitian.sigs](https://github.com/sub1x-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
