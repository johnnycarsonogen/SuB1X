...
pkgdatadir = $(datadir)/sub1x
pkgincludedir = $(includedir)/sub1x
pkglibdir = $(libdir)/sub1x
pkglibexecdir = $(libexecdir)/sub1x
...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/src/config/sub1x-config.h.in \
...
CONFIG_HEADER = $(top_builddir)/src/config/sub1x-config.h
...
ACLOCAL = ${SHELL} /root/sub1x/build-aux/missing aclocal-1.14
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/sub1x/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/sub1x/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/sub1x/build-aux/missing automake-1.14
AWK = mawk
BDB_CPPFLAGS = 
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/sub1x/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = sub1x
PACKAGE_BUGREPORT = www.sub1x.org
PACKAGE_NAME = Sub1x Core
PACKAGE_STRING = Sub1x Core 1.0.0
PACKAGE_TARNAME = sub1x
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/sub1x
abs_srcdir = /root/sub1x
abs_top_builddir = /root/sub1x
abs_top_srcdir = /root/sub1x
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/sub1x/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/sub1xd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/sub1x-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/sub1x-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = SUB1X-Qt.app
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_sub1x_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_sub1x_coverage.info test_sub1x.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/SUB1X-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/sub1x-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/sub1x-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/sub1x-config.h
$(top_srcdir)/src/config/sub1x-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/sub1x-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/SUB1X-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/SUB1X-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/SUB1X-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_sub1x.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_sub1x -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_sub1x_filtered.info: test_sub1x.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_sub1x_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_sub1x_coverage.info: baseline_filtered_combined.info test_sub1x_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_sub1x_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_sub1x_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_sub1x_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_sub1x.coverage/.dirstamp:  test_sub1x_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_sub1x.coverage/.dirstamp total.coverage/.dirstamp

# SUB1X: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_sub1x.coverage/ total.coverage/ $(OSX_APP)
