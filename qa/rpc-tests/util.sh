...
# JSON produced by sub1xd, do NOT use it to try to
...
function CreateDataDir {
  ...
  echo "rpcwait=1" >> $CONF
  echo "walletnotify=${SUB1XANDWAIT} -STOP" >> $CONF
  shift
  while (( "$#" )); do
      echo $1 >> $CONF
      shift
...
# Sub1x from to amount
function Sub1x {
  from=$1
  to=$2
  amount=$3
  address=$(Address $to)
  txid=$( ${SUB1XANDWAIT} $CLI $from sendtoaddress $address $amount )
}
...
# Use: Sub1xRawTxn <datadir> <hex_txn_data>
function Sub1xRawTxn {
  ${SUB1XANDWAIT} $CLI $1 sub1xrawtransaction $2
}
